...
import { __commonJS, __toESM, require_react } from './react-D7MT_jU0.js';
...
function createRouter(init) {
  ...
  async function handleFetcherAction(
    ...
    let actionResult = (await callDataStrategy(fetchRequest, fetchMatches, scopedContext, key))[
      match.route.id
    ];
  ...
  async function handleFetcherLoader(
    ...
    let result = (await callDataStrategy(fetchRequest, dsMatches, scopedContext, key))[
      match.route.id
    ];
    if (fetchControllers.get(key) === abortController) fetchControllers.delete(key);
    ...
    let fetcherResultsPromise = Promise.all(
      fetchersToLoad.map(async f => {
        if (f.matches && f.match && f.request && f.controller) {
          let result = (await callDataStrategy(f.request, f.matches, scopedContext, f.key))[
            f.match.route.id
          ];
          return { [f.key]: result };
        } else
...
function createStaticHandler(routes, opts) {
  ...
  async function submit(
    ...
    if (!actionMatch.route.action && !actionMatch.route.lazy) {
      ...
      result = (
        await callDataStrategy(request, dsMatches, isRouteRequest, requestContext, dataStrategy)
...
function findNearestBoundary(matches, routeId) {
  return (
    (routeId ? matches.slice(0, matches.findIndex(m => m.route.id === routeId) + 1) : [...matches])
      .reverse()
...
function RouterProvider({ router, flushSync: reactDomFlushSyncImpl, unstable_onError }) {
  ...
  let logErrorsAndSetState = import_react.useCallback(
    newState => {
...
var AwaitErrorBoundary = class extends import_react.Component {
  ...
  render() {
    ...
    if (status === 2)
      return /* @__PURE__ */ import_react.createElement(AwaitContext.Provider, {
        value: promise,
  ...
  while (stack.length > 0) {
    ...
    switch (typeof input2) {
      ...
      case 'object': {
        ...
        if (!pluginHandled) {
          ...
          if (isArray) {
            ...
            str[index2] =
              `["${TYPE_REGEXP}",${JSON.stringify(input2.source)},${JSON.stringify(input2.flags)}]`;
            ...
            if (input2.size > 0)
              str[index2] =
                `["${TYPE_MAP}",${[...input2].flatMap(([k, v]) => [flatten.call(this, k), flatten.call(this, v)]).join(',')}]`;
      ...
      default: {
        ...
        if (!pluginHandled)
          error = /* @__PURE__ */ new Error('Cannot encode function or unexpected type');
      }
    }
    if (error) {
      let pluginHandled = false;
...
`),
        ...
        controller.enqueue(
          textEncoder.encode(`[${encoder$1.stringified.join(',')}]
`),
        );
      ...
      if (Object.keys(encoder$1.deferred).length) {
        ...
        const racePromise = new Promise((resolve, reject) => {
          ...
          if (signal) {
            const rejectPromise = () =>
              reject(signal.reason || /* @__PURE__ */ new Error('Signal was aborted.'));
        ...
        while (Object.keys(encoder$1.deferred).length > 0) {
          for (const [deferredId, deferred] of Object.entries(encoder$1.deferred)) {
            ...
            seenPromises.add(
              (encoder$1.deferred[Number(deferredId)] = Promise.race([racePromise, deferred])
                .then(
                  resolved => {
                    const id2 = flatten.call(encoder$1, resolved);
                    if (Array.isArray(id2)) {
                      controller.enqueue(
                        textEncoder.encode(`${TYPE_PROMISE}${deferredId}:[["${TYPE_PREVIOUS_RESOLVED}",${id2[0]}]]
`),
                      );
                      encoder$1.index++;
                      lastSentIndex++;
...
`),
                      ...
                      controller.enqueue(
                        textEncoder.encode(`${TYPE_PROMISE}${deferredId}:[${values}]
`),
                      );
                      lastSentIndex = encoder$1.stringified.length - 1;
                    }
                  ...
                  reason => {
                    if (!reason || typeof reason !== 'object' || !(reason instanceof Error))
                      reason = /* @__PURE__ */ new Error('An unknown error occurred');
                    const id2 = flatten.call(encoder$1, reason);
                    if (Array.isArray(id2)) {
                      controller.enqueue(
                        textEncoder.encode(`${TYPE_ERROR}${deferredId}:[["${TYPE_PREVIOUS_RESOLVED}",${id2[0]}]]
`),
                      );
                      encoder$1.index++;
                      lastSentIndex++;
...
`),
                      ...
                      const values = encoder$1.stringified.slice(lastSentIndex + 1).join(',');
                      controller.enqueue(
                        textEncoder.encode(`${TYPE_ERROR}${deferredId}:[${values}]
`),
                      );
                      lastSentIndex = encoder$1.stringified.length - 1;
                    }
  ...
  if (done)
    ...
    return /* @__PURE__ */ import_react.createElement(
      ...
      /* @__PURE__ */ import_react.createElement(
        ...
        null,
        /* @__PURE__ */ import_react.createElement(StreamTransfer, {
          context,
    ...
    args.matches.map(async (m, i) =>
      m.resolve(async handler => {
        ...
        if (!m.unstable_shouldCallHandler(defaultShouldRevalidate)) {
          foundOptOutRoute ||
            (foundOptOutRoute =
              m.unstable_shouldRevalidateArgs != null && hasLoader && hasShouldRevalidate === true);
          return;
        }
    ...
    plugins: [
      (type, ...rest) => {
        if (type === 'SanitizedError') {
          ...
          if (name && name in global$1 && typeof global$1[name] === 'function')
...
function getRouteComponents(route, routeModule, isSpaMode) {
  ...
  let ErrorBoundary = routeModule.ErrorBoundary
    ...
    : route.id === 'root'
      ? () =>
          /* @__PURE__ */ import_react.createElement(RemixRootDefaultErrorBoundary, {
            error: useRouteError(),
          })
      : void 0;
  if (route.id === 'root' && routeModule.Layout)
    return {
      ...(Component4
        ? {
            element: /* @__PURE__ */ import_react.createElement(
              routeModule.Layout,
              null,
              /* @__PURE__ */ import_react.createElement(Component4, null),
            ),
          }
        : { Component: Component4 }),
      ...(ErrorBoundary
        ? {
            errorElement: /* @__PURE__ */ import_react.createElement(
              routeModule.Layout,
              null,
              /* @__PURE__ */ import_react.createElement(ErrorBoundary, null),
            ),
          }
        : { ErrorBoundary }),
      ...(HydrateFallback
        ? {
            hydrateFallbackElement: /* @__PURE__ */ import_react.createElement(
              routeModule.Layout,
              null,
              /* @__PURE__ */ import_react.createElement(HydrateFallback, null),
            ),
          }
        : { HydrateFallback }),
    };
...
function useFogOFWarDiscovery(router, manifest, routeModules, ssr, routeDiscovery, isSpaMode) {
  import_react.useEffect(() => {
    if (!isFogOfWarEnabled(routeDiscovery, ssr) || window.navigator?.connection?.saveData === true)
      return;
...
function Links({ nonce }) {
  ...
  let keyedLinks = import_react.useMemo(
  ...
  return /* @__PURE__ */ import_react.createElement(
    ...
    typeof criticalCss === 'string'
      ? /* @__PURE__ */ import_react.createElement('style', {
    ...
    keyedLinks.map(({ key, link }) =>
      isPageLinkDescriptor(link)
        ? /* @__PURE__ */ import_react.createElement(PrefetchPageLinks, {
            key,
            nonce,
            ...link,
          })
    ...
    meta.flat().map(metaProps => {
      ...
      if ('charSet' in metaProps && metaProps.charSet != null)
        return typeof metaProps.charSet === 'string'
          ? /* @__PURE__ */ import_react.createElement('meta', {
              key: 'charSet',
...
function Scripts(scriptProps) {
  ...
  let initialScripts = import_react.useMemo(() => {
    ...
    let contextScript = staticContext
      ? `window.__reactRouterContext = ${serverHandoffString};window.__reactRouterContext.stream = new ReadableStream({start(controller){window.__reactRouterContext.streamController = controller;}}).pipeThrough(new TextEncoderStream());`
      : ' ';
...
var RemixErrorBoundary = class extends import_react.Component {
  ...
  render() {
    if (this.state.error)
      return /* @__PURE__ */ import_react.createElement(RemixRootDefaultErrorBoundary, {
...
function BoundaryShell({ title, renderScripts, isOutsideRemixApp, children }) {
  ...
  return /* @__PURE__ */ import_react.createElement(
    ...
    /* @__PURE__ */ import_react.createElement(
      'head',
      null,
      /* @__PURE__ */ import_react.createElement('meta', { charSet: 'utf-8' }),
      /* @__PURE__ */ import_react.createElement('meta', {
        name: 'viewport',
    ...
    /* @__PURE__ */ import_react.createElement(
      ...
      /* @__PURE__ */ import_react.createElement(
        ...
        children,
        renderScripts ? /* @__PURE__ */ import_react.createElement(Scripts, null) : null,
      ),
    ),
  );
}
...
var Link = import_react.forwardRef(function LinkWithRef(
  ...
  return shouldPrefetch && !isAbsolute
    ? /* @__PURE__ */ import_react.createElement(
        ...
        null,
        link,
        /* @__PURE__ */ import_react.createElement(PrefetchPageLinks, { page: href$1 }),
      )
    : link;
});
...
function useSearchParams(defaultInit) {
  ...
  let navigate = useNavigate();
  let setSearchParams = import_react.useCallback(
...
function useFetcher({ key } = {}) {
  ...
  let load = import_react.useCallback(
    async (href$1, opts) => {
  ...
  let submit = import_react.useCallback(
    async (target, opts) => {
  ...
  let unstable_reset = import_react.useCallback(
...
function encodeLocation(to) {
  let href$1 = typeof to === 'string' ? to : createPath(to);
  href$1 = href$1.replace(/ $/, '%20');
  let encoded = ABSOLUTE_URL_REGEX3.test(href$1)
    ? new URL(href$1)
    : new URL(href$1, 'http://localhost');
  return {
...
var require_dist = /* @__PURE__ */ __commonJS({
  'node_modules/.pnpm/cookie@1.0.2/node_modules/cookie/dist/index.js': exports => {
    ...
    function serialize$1(name, val, options) {
      ...
      if (options.priority)
        switch (typeof options.priority === 'string' ? options.priority.toLowerCase() : void 0) {
          case 'low':
            str += '; Priority=Low';
            break;
          case 'medium':
...
var require_set_cookie = /* @__PURE__ */ __commonJS({
...
function createRoutesStub(routes, _context) {
  return function RoutesTestStub({ initialEntries, initialIndex, hydrationData, future }) {
    ...
    return /* @__PURE__ */ import_react.createElement(
      ...
      /* @__PURE__ */ import_react.createElement(RouterProvider, { router: routerRef.current }),
    );
  };
}
...
var createKey = async (secret, usages) =>
    ...
    async parse(cookieHeader, parseOptions) {
      ...
      if (name in cookies) {
        ...
        if (typeof value === 'string' && value !== '')
          return await decodeCookieValue(value, secrets);
        else return '';
      } else return null;
    },
    async serialize(value, serializeOptions) {
      return (0, import_dist.serialize)(
        name,
...
async function singleFetchLoaders(
  ...
  function staticContextToResponse(context) {
    ...
    if (context.errors)
      for (let [id, error] of Object.entries(context.errors)) results[id] = { error };
    for (let [id, data2] of Object.entries(context.loaderData))
      if (!(id in results) && loadedMatches.has(id)) results[id] = { data: data2 };
    return generateSingleFetchResponse(request, build, serverMode, {
      result: results,
      headers,
      status: context.statusCode,
...
async function handleResourceRequest(
  ...
  function handleQueryRouteError(error) {
    ...
    if (error instanceof Error && error.message === 'Expected a response from queryRoute') {
      let newError = /* @__PURE__ */ new Error(
...
function errorResponseToJson(errorResponse, serverMode) {
  return Response.json(
    serializeError(
      errorResponse.error || /* @__PURE__ */ new Error('Unexpected Server Error'),
      serverMode,
    ),
    {
...
function href(path, ...args) {
  ...
  let result = path
    .replace(/\/*\*?$/, '')
    .replace(/\/:([\w-]+)(\?)?/g, (_, param, questionMark) => {
...
async function routeRSCServerRequest({
  ...
  renderHTML,
  hydrate: hydrate$1 = true,
}) {
  const url = new URL(request.url);
  const respondWithRSCPayload =
    isReactServerRequest(url) || isManifestRequest(url) || request.headers.has('rsc-action-id');
  ...
  try {
    ...
    try {
      ...
      const html = await renderHTML(() => {
        const payloadPromise = Promise.resolve(createFromReadableStream(createStream())).then(
          payload =>
