...
An educational React + Vite application for browsing pizzas, filtering/sorting, and managing a simple cart. Built with TypeScript, React Router, SCSS modules, and ESLint. Uses Vite (rolldown-vite) for dev/build and pnpm as the package manager.
...
- Entry points:
  ...
  - src/router/router.ts (routes: Home, Cart; wrapped by Root layout)
- Styling: SCSS modules (with a global styles/index.scss). Vite is configured with an @ alias to src and a global mixins import for SCSS.
...
└─ src/
   ...
   ├─ router/
   │  └─ router.ts            # Route definitions (Home, Cart)
   ├─ pages/
   │  ├─ home.tsx
   │  └─ cart.tsx
...
- ESLint config (eslint.config.js): modern flat config using @eslint/js, typescript-eslint, react-hooks, and react-refresh rules.
...
- Port conflicts: set Vite port via a .env value (e.g., VITE_PORT) and configure server.port in vite.config.ts if needed.
